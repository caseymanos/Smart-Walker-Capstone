/*************************************************
   Created On: 2/3/22
   This is the main program which generates the output of both motors based on the distances read by all sensors
   Print statements enabled and pins set for Arduino as PCB was nonfunctional at time of writing
...
void objectDetection(int leftDistances[], int rightDistances[]) {
  ...
  */
  ...
  for (int j = 0; j < 5; j++) {
    ...
    if ((leftDistances[j] > minDist[j] && leftDistances[j] < maxDist[j] && j != 2) || (j==1 && (leftDistances[j] > minDist[j] && leftDistances[j] + 8 < maxDist[j]))) {
    ...
    if ((rightDistances[j] > minDist[j] && rightDistances[j] < maxDist[j] && j != 2) || (j==1 && (rightDistances[j] > minDist[j] && rightDistances[j] < (maxDist[j] +9 )))) {
