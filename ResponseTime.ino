/*************************************************
 * Created On: 2/3/22
 * This is the program for the response time test. It is essentially the same as the main code except data is printed to the serial monitor
...
#define model 100500; // for IR motor being used

ZSharpIR upperLeftIr(irUpLeft, model);
ZSharpIR lowerLeftIr (irLowLeft, model);
ZSharpIR upperRightIr(irUpRight, model);
ZSharpIR  lowerRightIr(irLowRight, model);
...
void loop() {
  ...
  */
  ...
  lowerLeftIrDist = lowerRightIr.distance();
  ...
  leftActualDist = findMin(leftDistances);
  rightActualDist = findMin(rightDistances);

  //generate PWM outputs 
  // left and right motor plot commands are in the functions
  leftMotorOutput(leftActualDist);
  rightMotorOutput(rightActualDist);

  // Print data to Serial Plotter
  
  Serial.print(ultraDist); // ultrasonic
  Serial.print("\t");

  Serial.print(leftTofDist);
  Serial.print("\t");

  Serial.print(rightTofDist); //right Tof
  Serial.print("\t");

  Serial.print(leftTofDist); // left Tof
  Serial.print("\t");

  Serial.print(upperLeftIrDist); // upper left IR
  Serial.print("\t");

  Serial.print(lowerLeftIrDist); // lower left IR
  Serial.print("\t");

  Serial.print(upperRightIrDist); // upper right IR
  Serial.print("\t");

  Serial.print(lowerLeftIrDist); // lower right IR
  Serial.print("\t");

  Serial.print(leftLidarDist); // left Lidar
  Serial.print("\t");

  Serial.println(rightLidarDist); // right Lidar
