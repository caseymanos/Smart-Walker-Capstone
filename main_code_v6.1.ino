  ...
  int rightTofReadings[WINDOW_SIZE];
    ...
    // Infrared Analog call
     void infraredSensorCall() {
      irPrevTime= millis();
      upperLeftIrDist = (upperLeftIr.getDistance())/100;
      lowerLeftIrDist = (lowerLeftIr.getDistance())/100;
      upperRightIrDist = (upperRightIr.getDistance())/100;
      lowerRightIrDist = (lowerRightIr.getDistance())/100;
...
int movingAverage(int &index, int &average, int &sum, int value, int readings[], int wSize){
  ...
  return average;
     ...
     // 
      ...
      for (int j =0; j<5; j++){
        ...
        if (leftDistances[j] > minDist[j] && leftDistances[j] < maxDist[j]) {
          ...
          Serial.print("left ");
          Serial.print(j);
          Serial.print(" is causing motor to turn on with distance ");
          Serial.println(leftDistances[j]);
          }
        }
        if (rightDistances[j] > minDist[j] && rightDistances[j] < maxDist[j]) {
          ...
          Serial.print(j);
          Serial.print(" is causing motor to turn on with distance ");
          Serial.println(rightDistances[j]);
    ...
    void leftMotorOutput(int leftPower, bool lHall) {
        ...
        Serial.print("left motor power is ===== ");
    ...
    void rightMotorOutput(int rightPower,bool rHall) {
        ...
        Serial.print("right motor power is ===== ");
